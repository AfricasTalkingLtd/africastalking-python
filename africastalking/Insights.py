...
class InsightService(Service):
    ...
    def _init_service(self):
        super(InsightService, self)._init_service()
    ...
    def check_sim_swap_state(self, phone_numbers, callback=None):
        ...
        if not all(validate_phone(phone_number) for phone_number in phone_numbers):
            return "Invalid phone number in given list: %s" % phone_numbers
        data = {
