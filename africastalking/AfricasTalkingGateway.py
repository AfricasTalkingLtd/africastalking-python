"""
 COPYRIGHT (C) 2017 AFRICASTALKING LTD <www.africastalking.com>                                                   #
...
http_worker = httplib2.Http('.cache')
...
class AfricasTalkingGateway:
    ...
    def fetchMessages(self, lastReceivedId_ = 0):
        url = "{}?username={}&lastReceivedId={}".format(self.getSmsUrl(), self.username, lastReceivedId_)
    ...
    def createSubscription(self, phoneNumber_, shortCode_, keyword_):
        ...
        url = "{}/create".format(self.getSmsSubscriptionUrl())
        parameters = {
    ...
    def deleteSubscription(self, phoneNumber_, shortCode_, keyword_):
        ...
        url = "{}/delete".format(self.getSmsSubscriptionUrl())
        parameters = {
            'username'     :self.username,
            'phoneNumber'  :phoneNumber_,
            'shortCode'    :shortCode_,
    ...
    def fetchPremiumSubscriptions(self,shortCode_, keyword_, lastReceivedId_ = 0):
        ...
        url = "{}?username={}&shortCode={}&keyword={}&lastReceivedId={}".format(self.getSmsSubscriptionUrl(),
                                                                               self.username,
                                                                               shortCode_,
                                                                               keyword_,
                                                                               lastReceivedId_)
    ...
    def call(self, from_, to_):
        ...
        url = "{}/call".format(self.getVoiceUrl())
        response = self.sendRequest(url, parameters)
        decoded  = json.loads(response)
    ...
    def getNumQueuedCalls(self, phoneNumber_, queueName_ = None):
        ...
        url      = "{}/queueStatus".format(self.getVoiceUrl())
        response = self.sendRequest(url, parameters)
        decoded  = json.loads(response)
        if decoded['errorMessage'] == "None":
            return decoded['entries']
    ...
    def uploadMediaFile(self, urlString_):
        ...
        url      = "{}/mediaUpload".format(self.getVoiceUrl())
        response = self.sendRequest(url, parameters)
        decoded  = json.loads(response)
        if decoded['errorMessage'] != "None":
            raise AfricasTalkingGatewayException(decoded['errorMessage'])
    ...
    def sendAirtime(self, recipients_):
        ...
        url      = "{}/send".format(self.getAirtimeUrl())
        response = self.sendRequest(url, parameters)
        decoded  = json.loads(response)
    ...
    def getUserData(self):
        url    = "{}?username={}".format(self.getUserDataUrl(), self.username)
        result = self.sendRequest(url)
        if self.responseCode == self.HTTP_RESPONSE_OK:
            decoded = json.loads(result)
    ...
    def sendRequest(self, urlString, data_ = None):
        try:
            ...
            if data_ is not None:
                ...
                response, content = http_worker.request(urlString, 'POST', data, headers = headers)
            else:
                response, content = http_worker.request(urlString, 'GET', headers = headers)
            ...
            if self.Debug:
            	#print "Raw response: " + content if python2
